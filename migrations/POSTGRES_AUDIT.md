...
Esta etapa **no implementa aún retención, particionamiento ni eliminación de índices**. Primero deja inventariados:
...
- no existe índice por `received_at`, por lo que una futura limpieza por fecha requeriría escaneos amplios si no se particiona;
...
El índice temporal todavía no participa en consultas de la API, pero es el
índice natural para una futura retención del ledger.
...
Crecimiento: una fila por batch aceptado, sin limpieza actual.
...
| Futura limpieza de ledgers | tablas `*_requests` | `created_at < límite` | índices `created_at desc` |
| Futura limpieza raw | tablas `*_raw` | `received_at < límite` | no cubierto actualmente |
...
4. Los índices temporales de los ledgers se usarán en la futura limpieza.
...
## 9. Orden recomendado después de obtener el reporte

1. Ejecutar la auditoría en desarrollo y en una copia representativa.
2. Guardar planes `EXPLAIN (ANALYZE, BUFFERS)` de las dos consultas públicas y
   de los CTE por `request_id`.
3. Definir horizontes separados para:
   - raw de precios;
   - raw de historial;
   - ledgers de idempotencia;
   - buckets históricos consolidados.
4. Elegir entre borrado por lotes y particionamiento por `received_at` para cada
   raw.
5. Diseñar una migración nueva que no bloquee innecesariamente escrituras.
6. Implementar mantenimiento, backup custom, checksum y restauración verificada.

## 10. Decisiones aún abiertas

- días exactos de retención por tabla;
...
- particiones diarias, semanales o mensuales;
- horizonte de `market_history_buckets` mayor, igual o menor a 366 días;
